...
void GPIO_PCLKControl(GPIO_RegDef_t *pGPIOx, uint8_t State)
{
	if (State == ENABLE)
	{
		if (pGPIOx == GPIOA) GPIOA_PCLK_EN(); else
		if (pGPIOx == GPIOB) GPIOB_PCLK_EN(); else
		if (pGPIOx == GPIOC) GPIOC_PCLK_EN(); else
		if (pGPIOx == GPIOD) GPIOD_PCLK_EN(); else
		if (pGPIOx == GPIOE) GPIOE_PCLK_EN();
	}
	else
	{
		if (pGPIOx == GPIOA) GPIOA_PCLK_DIS(); else
		if (pGPIOx == GPIOB) GPIOB_PCLK_DIS(); else
		if (pGPIOx == GPIOC) GPIOC_PCLK_DIS(); else
		if (pGPIOx == GPIOD) GPIOD_PCLK_DIS(); else
		if (pGPIOx == GPIOE) GPIOE_PCLK_DIS();
	}
...
void GPIO_Setup(GPIO_Handle_t *pGPIOHandle)
{
	...
	if (pGPIOHandle->GPIO_PinConfig.GPIO_PinMode == GPIO_MODE_ALTFN)
	{
		...
		pGPIOHandle->pGPIOx->AFR[tmp1] &= ~(0xF << (4 * tmp2)); //clearing
...
{
	if (pGPIOx == GPIOA) GPIOA_REG_RESET(); else
	if (pGPIOx == GPIOB) GPIOB_REG_RESET(); else
	if (pGPIOx == GPIOC) GPIOC_REG_RESET(); else
	if (pGPIOx == GPIOD) GPIOD_REG_RESET(); else
...
{
	if (Value == SET) pGPIOx->ODR |= (1 << PinNumber);
